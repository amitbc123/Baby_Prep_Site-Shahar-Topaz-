...
data class CategoryTotals(val category: ShoppingCategory, val estimated: Int, val spent: Int)
 ...
 */
fun itemEffectivePrice(item: ShoppingItem): Int? {
...
fun calculateBudget(items: List<ShoppingItem>): BudgetTotals {
    val byCategory = linkedMapOf<ShoppingCategory, MutableList<Int>>()
    var estimatedTotal = 0
    var spentTotal = 0
    var boughtCount = 0

    for (item in items) {
        val estimated = item.estimatedPrice ?: itemEffectivePrice(item) ?: 0
        val spent = if (item.status == ShoppingStatus.BOUGHT) itemEffectivePrice(item) ?: 0 else 0
        estimatedTotal += estimated
        spentTotal += spent
        if (item.status == ShoppingStatus.BOUGHT) boughtCount += 1

        val bucket = byCategory.getOrPut(item.category) { mutableListOf(0, 0) }
        bucket[0] += estimated
        bucket[1] += spent
    }
